...
Набор проверяет доменный гард, follow-up и исправления пользователя, чанкинг базы
знаний, BM25 и RRF, региональный retrieval, ветки деградации (hybrid → BM25,
main → fallback, отказ без обращения к LLM), лог оценок в SQLite и форматирование
температур.
